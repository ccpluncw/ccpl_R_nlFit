...
ordinalNumberLine <- function(targets, upperBound, lowerBound = 0, firstEstimate = NULL, memoryLength = NULL, rangeLength = 1, accuracyPercent = 0, numberSensitivity = 1, pIncludeConceptualPoints = 0, visibleReferencePoints = NULL, conceptualReferencePoints = NULL, verbose = FALSE) {
  ...
  halfRange <- numberSensitivity * upperBound

  numTargets <- length(targets)
  if (is.null(memoryLength)) memoryLength <- numTargets

  #add all information for targets except the actual estimate
  df.data <- data.frame(trial = seq(1, numTargets, 1), target = targets, low = targets - halfRange, high = targets + halfRange,  estimate = NA)
  ...
  #make visible reference points available
    ...
    if(numberSensitivityInfluencesVisbleReferencePoints) {
      ...
      df.reference <- data.frame(trial = 0, target = visibleReferencePoints, low = visibleReferencePoints - halfRange, high = visibleReferencePoints + halfRange, estimate =  visibleReferencePoints)
    } else {
