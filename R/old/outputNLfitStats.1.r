...
#' @param parList A list with the parameter names and values for the nlFit analysis. The list element name must be the parameter name and the value is the contents. The required elements are: firstEstimate; upperBound; lowerBound; rangeLength; memoryLength; targetOrder; accuracyPercent.
...
outputNLfitStats.1 <- function(data, statList, dataTargetCol = "target", dataEstimateCol = "estimate", pars.n = 1, loops = 1000, sinkFilename = NULL, appendSinkFile = TRUE, plotFileName = NULL) {

	df.fitted <- ordinalNumberLineFlex.1 (data[[dataTargetCol]], firstEstimate = statList$firstEstimate, upperBound = statList$upperBound, lowerBound =statList$lowerBound, rangeLength = statList$rangeLength, memoryLength = statList$memoryLength, loops = finalLoops, targetOrder = statList$targetOrder, accuracyPercent = statList$accuracyPercent)
  ...
  runStats <- list(parameters = parameters, fitStats = fitStats)
