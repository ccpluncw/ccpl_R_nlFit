...
getOrdinalNumberlineFit.1 <- function(data, upperBound, lowerBound = 0,firstEstimate = NULL, memoryLength = NULL, rangeLength = 2, accuracyPercent = 0, numberSensitivity = 1, referencePoints = NULL, loops = 1000, targetOrder = "random", dataTargetCol = "target", dataEstimateCol = "estimate", minimizeStat = 'BIC', pars.n = NULL) {

	parList <- list(upperBound = upperBound, lowerBound = lowerBound,firstEstimate = firstEstimate, memoryLength = memoryLength, rangeLength = rangeLength, numberSensitivity = numberSensitivity, referencePoints = referencePoints, accuracyPercent = accuracyPercent, targetOrder = targetOrder)
  ...
  if(validParams == TRUE) {
		...
		df.fitted <- ordinalNumberLineFlex.1(data[[dataTargetCol]], parList, loops=loops)
