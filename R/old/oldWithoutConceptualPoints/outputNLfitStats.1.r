...
outputNLfitStats.1 <- function(data, statList, dataTargetCol = "target", dataEstimateCol = "estimate", pars.n = 1, loops = 1000, sinkFilename = NULL, appendSinkFile = TRUE, plotFileName = NULL, xlim = NULL, ylim = NULL) {
	...
	if(is.null(xlim)) xLim <- c(statList$lowerBound, statList$upperBound)
	if(is.null(ylim)) yLim <- c(statList$lowerBound, statList$upperBound)
	pdf(plotFileName)
