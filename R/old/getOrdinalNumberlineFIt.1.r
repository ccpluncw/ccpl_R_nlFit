...
getOrdinalNumberlineFit.1 <- function(data, upperBound, lowerBound = 0,firstEstimate = NULL, memoryLength = NULL, rangeLength = 2, accuracyPercent = 0, loops = 1000, targetOrder = "random", dataTargetCol = "target", dataEstimateCol = "estimate", minimizeStat = 'BIC', pars.n = NULL) {

	parList <- list(upperBound = upperBound, lowerBound = lowerBound,firstEstimate = firstEstimate, memoryLength = memoryLength, rangeLength = rangeLength, accuracyPercent = accuracyPercent, loops = loops, targetOrder = targetOrder)

  if(is.null(pars.n)) {
    pars.n = length((pars.n))
  }
  ...
  if(validParams == TRUE) {
		...
		df.fitted <- ordinalNumberLineFlex.1 (data[[dataTargetCol]], firstEstimate = parList[["firstEstimate"]], upperBound = parList[["upperBound"]], lowerBound =parList[["lowerBound"]], rangeLength = parList[["rangeLength"]], memoryLength = parList[["memoryLength"]], loops = parList[["loops"]], targetOrder = parList[["targetOrder"]], accuracyPercent = parList[["accuracyPercent"]])
